...
login.post(function(req, res, next) {
    ...
    req.getConnection(function(err, conn) {
        ...
        let sql =
            "SELECT id,password,vtime,DATE_ADD( vtime, INTERVAL 1 DAY ) AS ctime,token FROM user WHERE phone = ? ";
        ...
        conn.query(sql, [phone], function(err, rows1) {
            ...
            if (rows1.length == 0) {
                ...
                let content = { msg: phone, time: Date.now(), id: rows1[0].id }, // 要生成token的主题信息
                    secretOrPrivateKey = "laoyouquan", // 这是加密的key（密钥）
                    token = jwt.sign(content, secretOrPrivateKey, {
                        //expiresIn: 60 * 60 * 24 // 24小时过期
                    });

                cursql += "UPDATE `user` SET token = '" + token + "' WHERE id = " + rows1[0].id;
